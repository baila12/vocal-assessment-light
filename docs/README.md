...
> **v7.14 | 2026-08-09** | 分支: `main`
> 上传音频自动匹配标准歌曲 (song_match DDD 子域: BPM/调性/chroma/duration 特征 + 确定性置信度 + POST /songs/match + upload auto_match + CompareView 自动匹配区) | 633 后端测试 GREEN + 297 前端 GREEN
...
| 层级 | 测试数 | 通过率 | 说明 |
|------|:-----:|--------|------|
| DDD 单元测试 (domain + infrastructure + alignment + flag + middleware) | 541 | ✅ 100% | 6 scorers + 音色调整 + 10 extractors + songs/songs_pitch/song_match domain + ScoringWeights + ABI + middleware |
| FastAPI 集成测试 | 71 | ✅ 100% | test_api_routes (19) + test_songs_api (20) + scoring API (14) + songs_pitch_api (9) + compare_pitch_api (3) + song_match_api (6, v7.14) (独立进程) |
...
| BDD | 18 step files, 21 feature files | ✅ | upload 5P+3S, animations 7P+9X, sing-song-select 6P+6X, pitch-realtime 25X, auto-match 5P+3X (v7.14) |
...
| [4-process/TEST_RESULTS.md](4-process/TEST_RESULTS.md) | 测试结果记录 (v7.14: 604 tests) |
...
| 测试变化 | 生产 537→633 (+96: 领域+79 基建+11 集成+6), 前端 286→297 (+11) | 多个测试文件 |
