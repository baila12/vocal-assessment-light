...
> - "节奏分析工具选择错误 (librosa.beat_track)" → v5.15 CV 重校准替代 ✅
> - "情绪评分域不匹配 (Wav2Vec2)" → v5.12 移除 + v5.14 复合评分替代 ✅
> - "魔法数字审计" 7个硬编码阈值 → v5.12 已集中到 `EmpiricalThresholds` ✅
> 
> 本文档保留作为算法设计参考。当前算法状态见 [SCORING.md](SCORING.md)。
