...
class ScoreServiceV4:
    ...
    def __init__(
        ...
        # 初始化各维度评分器
        self._pitch_scorer = PitchScorer(self._config.pitch)
        self._rhythm_scorer = RhythmScorer(self._config.rhythm)
        self._breath_scorer = BreathScorer(self._config.breath)
        self._technique_scorer = TechniqueScorer(
            self._config.technique, singing_style
        )
    ...
    def set_config(self, config: ScoringConfig) -> None:
        ...
        self._config = config
        # 更新各评分器的配置
        self._pitch_scorer = PitchScorer(config.pitch)
        self._rhythm_scorer = RhythmScorer(config.rhythm)
        self._breath_scorer = BreathScorer(config.breath)
        self._technique_scorer = TechniqueScorer(config.technique, self.singing_style)
    ...
    def calculate(
        ...
        # 8. 人声质量惩罚（v5.0 降低惩罚力度）
        if voice_quality_score < 50:
            quality_penalty = (50 - voice_quality_score) / 50 * 25
